...
- Cada operación HTTP tiene su propio **signal trigger** privado (`#params`)
- Cada operación HTTP tiene su propio **httpResource** público (`readonly`)
- Los métodos públicos hacen `.set()` en el signal para disparar la petición
- Cuando el signal es `null`, el httpResource retorna `undefined` = no fetch

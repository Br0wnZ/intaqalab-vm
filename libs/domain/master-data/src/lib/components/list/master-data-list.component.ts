  ...
  template: `
    @if (isLoading()) {
      <div class="flex flex-col gap-4 p-4">
        <ui-skeleton-table animation="wave" [columns]="columnsIds().length" [rows]="pageSize()" />
      </div>
    } @else if (hasError()) {
      <ui-error-state
        [title]="'MASTER_DATA.ERRORS.LOAD_FAILED_TITLE' | translate"
        [message]="'MASTER_DATA.ERRORS.LOAD_FAILED_DETAIL' | translate"
      />
    } @else {
...
export class MasterDataListComponent {
  ...
  protected createRecord(): void {
    this.#dialog.open(this.modalComponent, { ...this.#dialogStylesConfig, data: null });
  }

  protected onClickEdit(itemToEdit: MasterDataResponseType): void {
  ...
  protected onClickDelete(id: string): void {
    ...
    this.#dialog.open(MasterDataRemoveDialogComponent, {
      ...this.#dialogStylesConfig,

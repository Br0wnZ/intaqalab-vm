...
import type { EventLogGeneralDataFilters, EventLogGeneralDataSearch } from '../../../utils-models/general-data.model';
import { castEventLogFiltersToEventLogSearch } from '../../../utils-models/utils.model';

@Component({
  selector: 'inta-event-log-general-data-filter',
  imports: [
      ...
      <div class="w-full flex gap-4 flex-wrap">
        ...
        <div>
          <label for="event-filter-value" class="block text-sm font-medium text-gray-700 mb-2">
            {{ 'EVENT_LOG.GENERAL_DATA.FILTERS.VALUE.LABEL' | translate }}
          </label>
          <mat-form-field appearance="outline" class="w-full">
            <input
              ...
              [placeholder]="'EVENT_LOG.GENERAL_DATA.FILTERS.VALUE.PLACEHOLDER' | translate"
            />
          </mat-form-field>
        </div>
      </div>
...
export class EventLogGeneralDataFilterComponent {
  readonly #generalDataService = inject(GeneralDataService);
  ...
  readonly defaultFormValues: EventLogGeneralDataFilters = {
    dateStart: '',
    dateEnd: '',
    action: [],
    user: [],
  ...
  readonly formModel = signal<EventLogGeneralDataFilters>(this.defaultFormValues);
  ...
  readonly form = form(this.formModel, (schemaPath) => {
    validate(schemaPath, (ctx) => {
      ...
      if (dateRangeFilled || actionFilled || userFilled || hardwareIdFilled || valueFilled) return undefined;
  ...
  protected search() {
    ...
    this.#generalDataService.filtersItems.set(mappedFilters);
  }

  protected clearFilters() {
    const dataToSend: Partial<EventLogGeneralDataSearch> = {};

    this.#generalDataService.filtersItems.set(dataToSend);

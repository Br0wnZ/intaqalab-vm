...
import { EventLogGeneralDataStore } from '../../+state/general-data.store';
import { EventLogGeneralDataFilterComponent } from './general-data-filter/general-data-filter.component';

@Component({
  selector: 'inta-event-log-general-data',
  imports: [TranslateModule, MatTableModule, MatPaginatorModule, MatSortModule, EventLogGeneralDataFilterComponent],
  providers: [EventLogGeneralDataStore],
  template: `
    <div class="overflow-x-auto">
      <inta-event-log-general-data-filter />
      @if (store.items() && !store.isLoading()) {
        <table mat-table matSort class="w-full" [dataSource]="store.items()" (matSortChange)="onSort($event)">
          ...
          <ng-container matColumnDef="action">
            ...
            <td *matCellDef="let data" mat-cell class="px-6 py-4 text-sm text-gray-900 !bg-white">
              {{ 'EVENT_LOG.GENERAL_DATA.ACTIONS.' + data.action.toUpperCase() | translate }}
            </td>
          </ng-container>
          ...
          <ng-container matColumnDef="value">
            <th *matHeaderCellDef mat-sort-header mat-header-cell class="text-xs font-medium text-gray-600 px-6 py-3 !bg-gray-100">
              {{ 'EVENT_LOG.GENERAL_DATA.LABELS.VALUE' | translate }}
            </th>
            <td *matCellDef="let data" mat-cell class="px-6 py-4 text-sm text-gray-900 !bg-white">
              {{ data.value }}
            </td>
...
export class EventLogGeneralDataComponent {
  readonly store = inject(EventLogGeneralDataStore);

  readonly displayedColumns = ['date', 'action', 'user', 'hardwareId', 'value'];

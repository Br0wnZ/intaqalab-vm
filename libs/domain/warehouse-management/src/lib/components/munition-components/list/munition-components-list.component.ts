...
const DEFAULT_COLUMNS = ['name', 'category', 'observations', 'actions'] as const;
...
export class MunitionComponentsListComponent {
  ...
  getCheckedState(item: MunitionComponentsModel) {
    return this.#overrideItemId() === item.id ? !item.active : item.active;
  }

  async toogleActive(item: MunitionComponentsModel): Promise<void> {

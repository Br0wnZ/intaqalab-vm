...
export class StockListComponent {
  ...
  navigate(item: MunitionStockListResponse) {
    if (item.category === 'MUNITION') {
      this.#router.navigateByUrl(`/warehouse-management/stock/munitions/${item.id}`);
    } else {
      this.#router.navigateByUrl(`/warehouse-management/stock/munitions-components/${item.id}`);
    }
  }

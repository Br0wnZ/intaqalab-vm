...
import { Badge, ErrorState, IntaIconComponent, Skeleton } from '@intaqalab/ui';
    ...
    @if (isLoadingView()) {
      <div class="bg-white rounded-lg shadow-sm border border-gray-200 overflow-auto mt-4">
        ...
        <div class="grid grid-cols-[200px_120px_120px_1fr_100px] gap-4 px-6 py-3 bg-gray-50 border-b border-gray-200">
          ...
          <ui-skeleton variant="text" width="90px" height="1rem" animation="wave" />
          <ui-skeleton variant="text" width="100px" height="1rem" animation="wave" />
          <ui-skeleton variant="text" width="110px" height="1rem" animation="wave" />
          <ui-skeleton variant="text" width="60px" height="1rem" animation="wave" />
        </div>

        <!-- Series Rows skeleton -->
        <div class="flex flex-col">
          @for (i of 3 | range; track i) {
            <div class="border-b border-gray-200">
              <div
                class="grid grid-cols-[200px_120px_120px_1fr_100px] gap-4 items-center px-6 py-4"
                [class.bg-purple-100]="i === 0"
                [class.bg-white]="i !== 0"
              >
                ...
                <ui-skeleton variant="text" width="30px" height="1.25rem" animation="wave" />
                <ui-skeleton variant="text" width="30px" height="1.25rem" animation="wave" />
                <ui-skeleton variant="text" width="70%" height="1.25rem" animation="wave" />
              ...
              @if (i === 0) {
                <div class="bg-purple-50 p-6 space-y-4 border-t border-gray-300">
                  ...
                  <div class="grid grid-cols-[120px_1fr_120px] gap-4 py-2 border-b border-gray-300">
                    <ui-skeleton variant="text" width="80px" height="1rem" animation="wave" />
                    <ui-skeleton variant="text" width="100px" height="1rem" animation="wave" />
                    <ui-skeleton variant="text" width="60px" height="1rem" animation="wave" />
                  </div>
                  @for (j of 2 | range; track j) {
                    <div class="grid grid-cols-[120px_1fr_120px] gap-4 items-center py-3 border-b border-gray-200">
                      <ui-skeleton variant="text" width="30px" height="1.25rem" animation="wave" />
      ...
      <ui-error-state
        [title]="'TRIAL_PLANNING.SERIES_AND_SHOTS_SECTION.ERRORS.LOAD_FAILED_TITLE' | translate"
        [message]="'TRIAL_PLANNING.SERIES_AND_SHOTS_SECTION.ERRORS.LOAD_FAILED_DETAIL' | translate"
      />
    } @else {
      <div class="bg-white rounded-lg shadow-sm border border-gray-200 overflow-auto mt-4">
        <div class="flex justify-between items-center px-6 py-4 border-b border-gray-200 bg-white">
...
export class SeriesAndShots {
  ...
  protected readonly seriesStore = this.#seriesStore;

  readonly isLoadingView = computed(() => this.#seriesStore.isLoadingSeries() || this.#store.isLoadingPlanningInfo());
  ...
  async updateSerie(serieId: string) {
    if (this.readonly()) {
      return;
    }
  ...
  async deleteShot(serieId: string, shotId: string) {
    if (this.readonly()) {
      return;
    }
    const dialogRef = this.dialog.open(ConfirmDeleteShotDialog, {
      viewContainerRef: this.#viewContainerRef,
      data: {
        serieId,
        shotId,
  ...
  async deleteSerie(serieId: string) {
    ...
    const dialogRef = this.dialog.open(ConfirmDeleteSerieDialog, {
      viewContainerRef: this.#viewContainerRef,
      data: {
        serieId,

...
export class VelocityIntroduction extends BaseFormWidgetComponent {
  ...
  readonly #selectionKey = computed(
    () => `${this.selectorFormModel().serie ?? ''}|${this.selectorFormModel().disparo ?? ''}`,
  );
  ...
  protected readonly isDirty = computed(() => {
    ...
    return (
      this.dataFormModel().radarAntena !== snap.radarAntena ||
  ...
  async saveForm(): Promise<void> {
    ...
    if (fireTrialId && serie && disparo) {
      ...
      this.#executionService.setShotVelocity(fireTrialId, serie, disparo, payload);
    }
  ...
  async #loadSelectedShotData(): Promise<void> {
    ...
    try {
      ...
      if (this.#loadRequestVersion() !== requestVersion || this.#selectionKey() !== selectionKey) {
        return;
      }

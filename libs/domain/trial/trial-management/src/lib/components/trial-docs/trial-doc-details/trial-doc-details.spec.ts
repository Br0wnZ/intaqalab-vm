...
async function setup(options: { documentId?: string; mockDialog?: ReturnType<typeof createMockMatDialog> } = {}) {
  const mockService = makeMockDocsService();
  const mockDialog = options.mockDialog ?? createMockMatDialog();

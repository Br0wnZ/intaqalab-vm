...
@Component({
  ...
  template: `
    <div class="relative w-full">
      @if (label()) {
        <span
          class="absolute top-0 left-3 z-10 px-1 bg-white text-xs font-medium text-slate-500 leading-none select-none pointer-events-none -translate-y-1/2"
        >
          {{ label() }}
        </span>
      }
      <div
        class="flex items-center w-full bg-white rounded-xl border border-slate-200 focus-within:border-violet-500 focus-within:ring-2 focus-within:ring-violet-100/50 transition-all px-4 py-2 gap-3 h-12"
      >
        <!-- Campo X -->
        ...
        <mat-select
          ...
          disableRipple
          class="shrink-0 !w-max text-sm font-medium text-slate-600 bg-transparent"
          [value]="unit()"
          [disabled]="disabled()"
